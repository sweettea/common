...
sub new
{
  ...
  $self->{pid} = open3(\*IN,\*OUT,\*ERR,$cmd);
  ($self->{pid}) || &{$self->{'handler'}}($!);
...
sub rx
{
  ...
  while (!select(undef,undef,my $eout=$ein,0))  # while !eof()
  {
    ...
    while (!select(undef,undef,my $eout=$ein,0))  # while !eof()
    {
      ...
      # read one char
      my $outc;
      my $ret = sysread($self->{$handle},$outc,1);
      if (!$ret) {
	my $msg;
	if (defined($ret)) {
	  $msg = "eof read error";
	...
	$msg .= " from $handle (already read: $out)";
	if ($handle ne 'stderr') {
	  # This will in most cases not return since stderr is likely
	  # to be at EOF.
	  $msg .= ", stderr: " . $self->rx('stderr', $eot, $timeout);
	}
	&{$self->{'handler'}}($msg);
      }
...
Kill the child process and then reap it.  This will prevent lots of
defunct processes from accumulating.
